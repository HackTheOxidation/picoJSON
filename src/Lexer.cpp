...
namespace picoJSON {
  Lexer::Lexer(string json) : json_(json) {
    tokenize();
  }
          ...
          if (c == EOF) 
            ...
            if (isString)
              ...
              if (isNumber) {
                ...
                if (buffer[buffer.length() - 1] == '-' || buffer[buffer.length() - 1] == '+' || buffer[buffer.length() - 1] == '.')
                  throw SyntaxException(buffer, line);
            ...
            if ((c == 'e' || c == 'E') && isNumber && !exponent) {
              ...
              if (isNumber && (buffer[buffer.length() - 1] == 'e' || buffer[buffer.length() - 1] == 'E')) {
                buffer += c; 
              } else 
                throw SyntaxException(buffer, line);
            } else if (c == '+' && isNumber) {
